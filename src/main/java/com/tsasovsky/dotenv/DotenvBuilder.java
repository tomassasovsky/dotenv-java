...
public class DotenvBuilder {
     ...
     */
    public Dotenv load(Context context) throws DotenvException {
        final AssetManager assetManager = context.getAssets();

        final DotenvParser reader = new DotenvParser(
                new DotenvReader(directoryPath, filename),
                throwIfMissing, throwIfMalformed);
        final List<DotenvEntry> env = reader.parse(assetManager);
        if (systemProperties) {
    ...
    static class DotenvImpl implements Dotenv {
        ...
        @Override
        public Set<DotenvEntry> entries(final Filter filter) {
